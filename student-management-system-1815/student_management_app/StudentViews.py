...
def student_home(request):
    ...
    attendance_present = AttendanceReport.objects.filter(
        student_id=student_obj, status=1).count()
    attendance_absent = AttendanceReport.objects.filter(
        student_id=student_obj, status=0).count()
    ...
    return render(request, "student_template/student_home_template.html", {"total_attendance": total_attendance, "attendance_absent": attendance_absent, "attendance_present": attendance_present, "subjects": subjects, "data_name": subject_name, "data1": data_present, "data2": data_absent})
...
def student_view_attendance(request):
    ...
    if request.method == "POST":
        ...
        attendance_reports = AttendanceReport.objects.filter(
            attendance_id__in=attendance, student_id=stud_obj)  # attendance er moddhe multiple ache. tai __in use koreche.
        return render(request, 'student_template/student_attendance_data.html', context={"attendance_reports": attendance_reports})

    return render(request, "student_template/student_view_attendance.html", context={"subjects": subjects})
